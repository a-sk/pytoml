...
def loads(s, filename='<string>', translate=lambda t, x, v: v, _dict=dict):
    ...
    ast = _p_toml(src, _dict)

    def error(msg):
        raise TomlError(msg, pos[0], pos[1], filename)

    def process_value(v, _dict):
        ...
        if kind == 'array':
            ...
            value = [process_value(item, _dict) for item in value]
        elif kind == 'table':
            value = _dict([(k, process_value(value[k], _dict)) for k in value])
